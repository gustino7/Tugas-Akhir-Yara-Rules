...
def main(input_file, malware_fam, min_length=5, max_length=20):
    ...
    output_dir = './output_lcs'
    os.makedirs(output_dir, exist_ok=True)
    
    # Find strings common to all files
    print(f"Finding strings common to all files with minimum length {min_length}...")
    all_files_result = find_strings_in_all_files(data, min_length, max_length)
    
    # Save results
    output_file = os.path.join(output_dir, f'String_Matched_{malware_fam}.json')
    with open(output_file, 'w') as f:
        json.dump(all_files_result, f, indent=2)
    
    # Print summary
    print(f"Results saved to {output_file}")

if __name__ == "__main__":
    input_file = "./output_parser/String_Vidar.json"
    malware_fam = "Vidar"
    min_length = 5
    
    main(input_file, malware_fam, min_length)